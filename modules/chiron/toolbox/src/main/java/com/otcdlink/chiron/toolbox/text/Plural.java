...
public final class Plural {
  ...
  public static String of( final long cardinality, final String single, final String... more ) {
    if( cardinality <= 1 || more.length == 0 ) {
      return cardinality + " " + single ;
    } else {
      final int normalisedCount = Math.max( 0, ( ( int ) cardinality ) - 2 ) ;
      final int index = Math.min( normalisedCount, more.length - 1 ) ;
